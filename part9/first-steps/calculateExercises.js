export const argumentsValue = (hoursArgs, targetVal) => {
    if (hoursArgs.length < 2)
        throw new Error('Not enough arguments');
    if (hoursArgs.length > 8)
    ...
    const hours = hoursArgs.map(value => {
        ...
        if (isNaN(num))
            throw new Error("All hours must be valid numbers");
        return num;
    });
    return { hours, target };
};
export const calculateExercise = (target, hours) => {
